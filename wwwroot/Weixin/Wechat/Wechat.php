...
class Wechat
{
	...
	public function __construct($options)
	{
		...
		$this->logpath = isset($options['logpath'])?$options['logpath']:'./wechat.log';
...
//		D($options);
    ...
    {
		$this->log('=====================请求开始 - '.date('Y-m-d H:i:s', time()).'=====================');
		$encryptStr="";
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        } elseif (isset($_GET["echostr"])) {				//要求输出str 验证
			$this->log('Get > ');
			$this->log($_GET);
        	...
        	if ($return) {
        		...
        		if ($this->checkSignature()){
					$this->log('return and end > 验证通过返回');		//验证通过返回
					$this->log($echoStr);		//验证通过返回
					$this->log('=====================请求结束 - '.date('Y-m-d H:i:s', time()).'====================='."\n");
					die($echoStr);
				}
        		else{
					$this->log('Die > access2 ');
					$this->log('=====================请求结束 - '.date('Y-m-d H:i:s', time()).'====================='."\n");
					die('no access2');
				}
        	}
    ...
    protected function log($log){
    		if ($this->debug && !empty($this->logcallback)) {
